...
class DynamicSignal:
    ...
    def __init__(self, data_pnts=None, label=None, color=None, is_normalized=False):
        DynamicSignal.signal_counter += 1
        self.ID = "Signal_" + str(DynamicSignal.signal_counter)
        self.label = label
        self.data_qpnts = []
        self.data_pnts = []
        self.shift = 0
        self.last_point = 0
        DynamicSignal.signals = []
        if color is None:
    ...
    def from_file(file_path, is_normalized=False):
        ...
        try:
            ...
            return None
        return DynamicSignal(data_pnts, is_normalized=is_normalized, label=file_path.split('/')[-1].split('.')[0], color="#{:06x}".format(np.random.randint(0, 0xFFFFFF)))
